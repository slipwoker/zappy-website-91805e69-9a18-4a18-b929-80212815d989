document.addEventListener('DOMContentLoaded', function () {
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      fetch(action, {
        ...
        .then(function (response) {
          if (response.ok) {
            contactForm.reset();
            submitFields.forEach(function (field) { clearError(field); });
        ...
        .catch(function (err) {
          successMessage.textContent = err.message || 'Something went wrong. Please try again later.';
          successMessage.style.display = 'block';
          successMessage.classList.add('form-error-banner');
        })
        .finally(function () {
          if (submitBtn) {
  ...
  const animationTargets = document.querySelectorAll(
    '.fade-in, .fade-in-element, [data-animate], ' +
    '.feature-card, .service-card, .pricing-card, .testimonial-card, ' +
    '.hero-content, .section-header, .cta-block, .about-content, ' +
    '.team-member, .portfolio-item, .blog-card, .stat-item'
  );
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "אקסלנס טרייד משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '91805e69-9a18-4a18-b929-80212815d989',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '91805e69-9a18-4a18-b929-80212815d989';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '91805e69-9a18-4a18-b929-80212815d989';
